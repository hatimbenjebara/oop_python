#oop python revision 
#first thing we need to know if you want an objet you need a class
#structure like integer floaat is build types 
...
class computer :
    ...
    def __init__(self): #to initialise variable
        print("in init") #for automatique, for each object its do it one time 
...
computer.config(b) #
b2 =computer()
b2.config() # its same as computer.config(b) and this senteces must used 
class name :   
